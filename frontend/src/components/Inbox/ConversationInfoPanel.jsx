...
export default function ConversationInfoPanel({ conversation }) {
  ...
  const updateContact = async (field, value) => {
    try {
      ...
      setContact(r.data || r); toast.success('Guardado');
  ...
  const saveCustomFields = async (fields) => {
    ...
    try {
      ...
      const c = r.data || r;
      setContact(c); setCustomFields(fields);
    } catch { toast.error('Error al guardar'); }
  };
  ...
  const patchConvMeta = async (metaPatch) => {
    try {
      await api.patch(`/conversations/${convId}`, { metadata: metaPatch });
  ...
  return (
    <div className="h-full overflow-y-auto bg-white flex flex-col">
      ...
      <Section title="Conversación" icon={MessageSquare} defaultOpen={false}>
        ...
        <InfoRow label="Ticket"   value={conversation?.ticket_number || conversation?.ticket_id} mono />
        <InfoRow label="ID"       value={`#${(conversation?.id || '').replace(/\D/g, '').slice(0, 6)}`} mono />
        {conversation?.metadata?.client_ip && (
          <>
            <InfoRow label="IP cliente" value={conversation.metadata.client_ip} mono />
            <button
              onClick={async () => {
                if (!window.confirm(`¿Bloquear la IP ${conversation.metadata.client_ip}? No podrá usar el widget.`)) return;
                try {
                  await api.post('/widget/block-ip', { ip: conversation.metadata.client_ip });
                  toast.success(`IP ${conversation.metadata.client_ip} bloqueada`);
                } catch { toast.error('Error bloqueando IP'); }
              }}
