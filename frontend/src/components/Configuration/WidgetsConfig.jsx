    ...
    const load = async () => {
      try {
        ...
        if (data) {
          ...
          setTitle(data.nombre || 'Chat con nosotros');
          const wc = data.widget_config;
          if (wc) {
            if (wc.color) setColor(wc.color);
            if (wc.title) setTitle(wc.title);
            if (wc.subtitle) setSubtitle(wc.subtitle);
            if (wc.position) setPosition(wc.position);
            if (wc.preFormEnabled !== undefined) setPreFormEnabled(wc.preFormEnabled);
            if (wc.preFormTitle) setPreFormTitle(wc.preFormTitle);
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put('/company', {
        widget_config: { color, title, subtitle, position, preFormEnabled, preFormTitle, fields }
      });
