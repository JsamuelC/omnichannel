...
import { Zap, X, ClipboardList, FileText, FolderOpen } from 'lucide-react';
import { useConversationStore, useAuthStore, useThemeStore, useModuleStore } from '../../store';
import { confirmAction } from '../../utils/confirmDialog.jsx';
...
export default function ChatWindow() {
  ...
  const handleSendCatalog = async (catalog) => {
    setShowCatalogPicker(false);
    try {
      const content = catalog.tipo === 'text' ? catalog.contenido : `📎 ${catalog.nombre}`;
      await sendMessage(content);
      toast.success(`Catálogo "${catalog.nombre}" enviado`);
    } catch (err) { toast.error('Error enviando catálogo'); }
  };

  const handleUseTemplate = async (template) => {
    setShowTemplatePicker(false);
    try {
      const res = await api.post(`/merge-templates/${template.id}/use/${activeConversation.id}`, { datos: {} });
      if (res.data?.merged) {
        await sendMessage(res.data.merged);
        toast.success(`Plantilla "${template.nombre}" enviada`);
      } else {
        setText(template.contenido || '');
        toast('Plantilla cargada — completa los campos y envía');
      }
    } catch (err) { toast.error('Error usando plantilla'); }
  };
  ...
  const handleDelete = () => {
    confirmAction('¿Eliminar esta conversación y todos sus mensajes?', async () => {
      try {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pb-3 pt-2 bg-[#f0f2f5] dark:bg-[#202c33] border-t border-[#d1d7db] dark:border-[#2a3942] flex-shrink-0">
        ...
        {activeConversation.status === 'resolved' ? (
          <div className="flex items-center justify-center gap-2 py-3 px-4 bg-slate-100 dark:bg-slate-800 rounded-xl">
            <svg viewBox="0 0 20 20" className="w-4 h-4 text-slate-400" fill="currentColor">
              <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
            </svg>
            <span className="text-sm text-slate-500 dark:text-slate-400">Esta conversación fue finalizada</span>
          </div>
        ) : (
        <form onSubmit={handleSend} className="flex items-end gap-2">
            ...
            <div className="relative flex-shrink-0">
              <button type="button" onClick={() => setShowCatalogPicker(p => !p)}
                className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors ${
                  showCatalogPicker ? 'bg-amber-500 text-white' : 'bg-white dark:bg-[#2a3942] text-[#667781] hover:text-amber-500 hover:bg-amber-50'
                }`} title="Enviar catálogo">
                <FolderOpen size={16} />
              </button>
              {showCatalogPicker && (
                <div className="absolute bottom-11 left-0 z-30 bg-white dark:bg-[#2a3942] rounded-xl shadow-xl border border-slate-100 dark:border-[#3b4a54] w-56 max-h-48 overflow-y-auto">
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider px-3 py-2 border-b border-slate-50 dark:border-[#3b4a54]">Catálogos</p>
                  {catalogs.map(c => (
                    <button key={c.id} type="button" onClick={() => handleSendCatalog(c)}
                      className="w-full text-left flex items-center gap-2 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-[#3b4a54] transition-colors">
                      <FolderOpen size={13} className="text-amber-500 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-700 dark:text-slate-200 font-medium truncate">{c.nombre}</p>
                        <p className="text-xs text-slate-400 truncate">{c.tipo}</p>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          {/* Botón plantillas dinámicas */}
          {templates.length > 0 && (
            <div className="relative flex-shrink-0">
              <button type="button" onClick={() => setShowTemplatePicker(p => !p)}
                className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors ${
                  showTemplatePicker ? 'bg-violet-500 text-white' : 'bg-white dark:bg-[#2a3942] text-[#667781] hover:text-violet-500 hover:bg-violet-50'
                }`} title="Plantillas dinámicas">
                <FileText size={16} />
              </button>
              {showTemplatePicker && (
                <div className="absolute bottom-11 left-0 z-30 bg-white dark:bg-[#2a3942] rounded-xl shadow-xl border border-slate-100 dark:border-[#3b4a54] w-56 max-h-48 overflow-y-auto">
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider px-3 py-2 border-b border-slate-50 dark:border-[#3b4a54]">Plantillas</p>
                  {templates.map(t => (
                    <button key={t.id} type="button" onClick={() => handleUseTemplate(t)}
                      className="w-full text-left flex items-center gap-2 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-[#3b4a54] transition-colors">
                      <FileText size={13} className="text-violet-500 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-700 dark:text-slate-200 font-medium truncate">{t.nombre}</p>
                        <p className="text-xs text-slate-400 truncate">{t.canal || 'todos'}</p>
                      </div>
                    </button>
                  ))}
                </div>
