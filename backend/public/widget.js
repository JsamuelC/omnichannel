(function() {
  ...
  // Cargar teléfono de la empresa
  fetch(API + '/widget/config/' + COMPANY).then(function(r) { return r.json(); }).then(function(d) {
    if (d.success && d.data) companyPhone = d.data.company_phone;
  }).catch(function() {});
  ...
  style.textContent = '\
    ...
    #ts-widget-end{padding:8px 16px;text-align:center;border-top:1px solid #1e293b}\
    #ts-widget-end button{background:none;color:#64748b;border:none;font-size:11px;cursor:pointer;padding:4px 8px;border-radius:6px;transition:all .2s}\
    #ts-widget-end button:hover{color:#ef4444;background:rgba(239,68,68,.1)}\
  ...
  function renderChat() {
    return renderHeader() + '<div id="ts-widget-msgs"></div><div id="ts-widget-input"><input id="ts-input" type="text" placeholder="Escribe tu mensaje..." autocomplete="off"><button id="ts-send">Enviar</button></div><div id="ts-widget-end"><button id="ts-end-chat">Finalizar chat</button></div>';
  }
  ...
  function showChat() {
    ...
    async function initSession() {
      try {
        ...
        if (data.success) {
          ...
          addMsg('Lo sentimos, no puedes iniciar una conversación en este momento.', 'in');
        }
    ...
    async function sendMessage() {
      ...
      if (!convId) {
        await initSession();
        // No cargar historial después del init para evitar duplicados
      }
      showTyping();
      try {
        ...
        if (data.success && data.data.botReply) {
          seenIds.add(data.data.botReply.id);
          addMsg(data.data.botReply.content, 'in', fmtTime(data.data.botReply.created_at));
          lastPollTime = data.data.botReply.created_at;
        }
      } catch (e) {
        hideTyping();
    ...
    var endBtn = document.getElementById('ts-end-chat');
    if (endBtn) endBtn.onclick = function() {
      if (typeof stopPolling === 'function') stopPolling();
      // Notificar al backend que el cliente cerró la conversación
      if (convId) {
        fetch(API + '/widget/end/' + convId, { method: 'POST' }).catch(function() {});
      }
      msgsEl.innerHTML = '';
      document.getElementById('ts-widget-input').style.display = 'none';
      document.getElementById('ts-widget-end').style.display = 'none';
      var bizPhone = companyPhone || null;
      var endHtml = '<div style="text-align:center;padding:24px 16px"><div style="font-size:32px;margin-bottom:12px">✅</div><p style="color:#e2e8f0;font-size:14px;font-weight:600;margin:0 0 6px">Chat finalizado</p><p style="color:#64748b;font-size:12px;margin:0 0 20px">Gracias por contactarnos.</p>';
      if (bizPhone) {
        var waNumber = bizPhone.replace(/[^0-9]/g, '');
        endHtml += '<a href="https://wa.me/' + waNumber + '" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:10px 20px;background:#25d366;color:#fff;text-decoration:none;border-radius:10px;font-size:13px;font-weight:600"><svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492l4.675-1.478A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818c-2.168 0-4.19-.592-5.926-1.625l-.424-.254-2.774.877.86-2.692-.276-.44A9.777 9.777 0 012.182 12c0-5.422 4.396-9.818 9.818-9.818 5.422 0 9.818 4.396 9.818 9.818 0 5.422-4.396 9.818-9.818 9.818z"/></svg>Continuar por WhatsApp</a>';
      }
      endHtml += '<button id="ts-new-chat" style="display:block;margin:12px auto 0;background:none;border:none;color:#64748b;font-size:12px;cursor:pointer;text-decoration:underline">Iniciar nuevo chat</button></div>';
      msgsEl.innerHTML = endHtml;
      msgsEl.style.display = 'flex';
      msgsEl.style.alignItems = 'center';
      msgsEl.style.justifyContent = 'center';
      var newChatBtn = document.getElementById('ts-new-chat');
      if (newChatBtn) newChatBtn.onclick = function() {
        sessionId = null; convId = null; formDone = false;
        localStorage.removeItem('ts_widget_session');
        localStorage.removeItem('ts_widget_conv');
        if (PRE_FORM && PRE_FORM.enabled) showPreForm();
        else showChat();
      };
    };
  ...
  function showPreForm() {
    ...
    submitBtn.onclick = function() {
      ...
      if (!valid) return;
      // Nueva sesión para este visitante
      sessionId = null;
      convId = null;
      localStorage.removeItem('ts_widget_session');
      localStorage.removeItem('ts_widget_conv');
