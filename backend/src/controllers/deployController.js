...
exports.getStatus = async (req, res) => {
  try {
    const ghApi = getGhApi();
    if (!ghApi) {
      return res.status(503).json({ success: false, error: 'Variables de GitHub no configuradas (GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO)' });
    }
...
exports.triggerQA = async (req, res) => {
  const ghApi = getGhApi();
  if (!ghApi) return res.status(503).json({ success: false, error: 'Variables de GitHub no configuradas' });
