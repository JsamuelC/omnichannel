...
exports.init = async (req, res) => {
  try {
    ...
    // Emitir evento de nueva conversación a los agentes para sincronizar bandeja
    const io = req.app.get('io');
    if (io && conversation.isNewRecord !== false) {
      const fullConv = await Conversation.findByPk(conversation.id, {
        include: [{ model: Contact, as: 'contact' }]
      });
      if (fullConv) {
        io.to('agents').emit('conversation:new', { conversation: fullConv.toJSON() });
      }
    }
