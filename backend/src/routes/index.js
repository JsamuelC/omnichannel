...
router.get('/health', (req, res) => {
  ...
  res.json({ status: 'ok', app: 'Tecnossync', timestamp: new Date().toISOString(), version: pkg.version, uptime: Math.floor(process.uptime()) });
});
...
// Cambiar disponibilidad propia (ANTES de :id para que no capture 'me')
router.patch('/users/me/availability', auth, async (req, res) => {
  try {
    const { availability } = req.body;
    if (!['active', 'inactive', 'break'].includes(availability)) {
      return res.status(400).json({ success: false, message: 'Estado inválido' });
    }
    const { User } = require('../models');
    await User.update({ availability }, { where: { id: req.user.id } });
    const io = req.app.get('io');
    if (io) io.to('agents').emit('user:availability', { userId: req.user.id, availability });
...
router.get('/notifications', auth, async (req, res) => {
  try {
    ...
    const where = {};
    if (req.user.company_id) where.company_id = req.user.company_id;
    else if (req.user.role !== 'superadmin') where.user_id = req.user.id;
    const { limit = 20, unread } = req.query;
    if (unread === 'true') where.read = false;
    const notifs = await Notification.findAll({ where, order: [['created_at', 'DESC']], limit: +limit });
    const unreadCount = await Notification.count({ where: { ...where, read: false } });
    res.json({ success: true, data: { notifications: notifs, unread_count: unreadCount } });
...
router.post('/notifications/read', auth, async (req, res) => {
  try {
    ...
    if (ids && Array.isArray(ids)) {
      await Notification.update({ read: true, read_at: new Date() }, { where: { id: ids } });
    } else {
      const where = {};
      if (req.user.company_id) where.company_id = req.user.company_id;
      await Notification.update({ read: true, read_at: new Date() }, { where: { ...where, read: false } });
    }
