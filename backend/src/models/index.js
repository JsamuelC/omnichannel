...
const migrate = async () => {
  ...
  try {
    ...
    // Nombres de tabla/columna vienen de este array hardcodeado (no se pueden parametrizar en DDL)
    const intToUuidCols = [['appointments','created_by'],['appointments','assigned_to'],['document_templates','created_by'],['module_records','created_by'],['module_records','conversation_id']];
    const validIdent = /^[a-z_]+$/;
    for (const [table, col] of intToUuidCols) {
      if (!validIdent.test(table) || !validIdent.test(col)) continue;
      try {
        ...
        if (check.length > 0) {
          ...
          await sequelize.query(`ALTER TABLE "${table}" ADD COLUMN "${col}" UUID`);
        }
    ...
    for (const feat of configFeatureKeys) {
      try {
        await sequelize.query(
          `UPDATE company SET active_features = active_features || jsonb_build_object($1, true)
           WHERE active_features IS NOT NULL AND NOT (active_features ? $1)`,
          { bind: [feat] }
        );
      } catch (_) {}
    }
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        for (const row of rows) {
          ...
          await sequelize.query(
            `UPDATE conversations SET ticket_number = $1 WHERE id = $2`,
            { bind: [tn, row.id] }
          );
        }
        logger.info(`ticket_number asignado a ${rows.length} conversaciones existentes`);
      }
    } catch (e) { logger.warn('No se pudo poblar ticket_number:', e.message); }
    ...
    try {
      ...
      if (firstCompany) {
        ...
        for (const { t, extra } of tablasPoblar) {
          try {
            await sequelize.query(
              `UPDATE "${t}" SET company_id = $1 WHERE company_id IS NULL ${extra}`,
              { bind: [cid] }
            );
